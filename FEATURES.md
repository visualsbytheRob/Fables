...
**Status:** Epic 17 (Audio Fables) underway — **TTS Foundation F1601–F1610 COMPLETE** ([x] shipped; F1604 Web Speech fallback [~] web-layer). Shipped: pluggable `TtsRuntime` + `TtsAdapter` (mirrors the AI runtime), `PiperAdapter` (local, offline, graceful when no binary), content-addressed synthesis cache with LRU budget + priority queue (migration 030), per-vault voice settings, SSML-ish markup + pronunciation lexicon, routes (GET /tts/status, GET/PUT /tts/settings, POST /tts/synthesize), and `docs/audio/tts.md`. **2,972 tests green across 258 files.** Next: F1611 (Voice Casting). Prior: Epic 16 (Canvas) server foundation complete; earlier Tier-2 epics 11 (plugins), 12 (CRDT/collab), 13 (vault core), 14 (AI), 15 (importers/interop). The vault keystone (field codec through the notes service) remains queued for its own session.
