...
**Status:** Epic 17 (Audio Fables) underway — **TTS Foundation F1601–F1610 + Voice Casting F1611–F1620 COMPLETE** ([x] shipped; F1604 Web Speech fallback + F1614 audition UI [~] web-layer). TTS: pluggable `TtsRuntime`/`TtsAdapter`, `PiperAdapter`, content-addressed synthesis cache + LRU + priority queue (migration 030), per-vault voice settings, SSML-ish markup + lexicon, /tts routes, `docs/audio/tts.md`. Casting: pure dialogue-attribution + narrator/character separation + cast resolution with fallback rules (`audio/casting/*`), per-entity voices + saved cast sheets + templates (migration 031), /casting + /stories/:id/cast + /entities/:id/voice routes. **3,027 tests green across 263 files.** Next: F1621 (Narration Renderer). Prior: Epic 16 (Canvas) server foundation complete; earlier Tier-2 epics 11–15. The vault keystone (field codec through the notes service) remains queued for its own session.
