...
**Status:** Epic 17 (Audio Fables) underway — **TTS Foundation F1601–F1610 + Voice Casting F1611–F1620 + Narration Renderer F1621–F1630 COMPLETE** ([x] server shipped; web-player items F1623/F1625/F1627/F1628 + F1604/F1614 [~]). TTS: pluggable runtime/adapter + Piper, content-addressed cache + LRU + priority queue (migration 030), settings, markup + lexicon, /tts routes, `docs/audio/tts.md`. Casting: pure dialogue-attribution + narrator/character separation + cast resolution (`audio/casting/*`), per-entity voices + cast sheets + templates (migration 031), /casting routes. Narration: Forge path → voiced audio scene + timeline, pre-render baking to one WAV + hand-written `wav.ts`, /narration routes. Soundscapes: scene-tag bindings + `play()` triggers from Forge source (`audio/soundscape/bindings.ts`/`triggers.ts`), bundled CC0 sound library + attribution manifest, mixer model + ducking + per-vault mix settings (migration 032), routes /soundscape/library + /attribution + /mix + POST /stories/:id/soundscape. Read-Along: word/sentence alignment model (`audio/readalong/align.ts`) — engine-boundary (F1642) + length-weighted proportional fallback (F1647), text-generic so it serves notes too (F1646), route POST /readalong/align. Recording Studio: content-addressed human-narration takes (sha256 dedup, migration 033) with take management (list/pick-best/promote-on-delete), mixed human+TTS recording plan + session checklist (`audio/studio/plan.ts`), routes /stories/:id/takes + /recording-plan + /takes/:id/audio. Audio Export: audiobook manifest (`audio/export/audiobook.ts`) — chapters from knot titles, embedded metadata, per-format size estimate, `.cue` sheet; routes POST /stories/:id/audiobook + /notebooks/:id/audiobook (note-to-audio). Playback System: cross-device resume positions, listening queue, offline pins, and listening stats (`db/repos/playback.ts`, migration 034), routes under /playback. Audio Accessibility: transcripts + WebVTT captions + numbered spoken choice menus (`audio/a11y/transcript.ts`), mono/balance/voice-normalization audio settings, routes POST /stories/:id/transcript + extended /soundscape/mix. **3,162 tests green across 285 files. CI green (fixed a pre-existing benchmark-timeout flake that was failing every push).** Next: F1691 (Audio Epic Close). Codec muxing + Media Session/Web Audio playback/editor/mic layers are deferred-with-reason. Prior: Epic 16 (Canvas) server foundation complete; earlier Tier-2 epics 11–15. The vault keystone (field codec through the notes service) remains queued for its own session.
