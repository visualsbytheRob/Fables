...
- [~] F1391 — AI settings page: backend, models, per-feature toggles — server settings store + kill switch + data-use explainer shipped; the page/badge/onboarding are the web UI pass
- [x] F1392 — Global AI kill switch
- [x] F1393 — Data-use explainer (what context each feature sees)
- [x] F1394 — Per-notebook AI exclusions (private areas)
- [x] F1395 — Secret notes always invisible to AI (enforced + tested)
- [~] F1396 — First-run AI onboarding with honest capability framing — server settings store + kill switch + data-use explainer shipped; the page/badge/onboarding are the web UI pass
- [~] F1397 — Local-only badge in UI during AI operations — server settings store + kill switch + data-use explainer shipped; the page/badge/onboarding are the web UI pass
- [x] F1398 — AI feature documentation
- [~] F1399 — Full AI suite e2e on demo vault — deferred: needs a seeded demo-vault fixture + browser e2e runner (with F1329/F1369 datasets)
- [x] F1400 — Epic 14 retro devlog (docs/devlog/epic-14.md)
...
- [x] F1451 — Bear export import (md + assets, tag syntax)
- [x] F1452 — Day One JSON import → journal with metadata
- [x] F1453 — Day One photos/locations/weather as note metadata
- [x] F1454 — Simplenote export import
- [x] F1455 — Google Keep takeout import
- [x] F1456 — Standard Notes export import
- [x] F1457 — Joplin export (JEX) import
- [x] F1458 — Generic folder-of-markdown enhancer (frontmatter dialects)
- [x] F1459 — Misc importer fixture tests
- [x] F1460 — Per-source guides

### Documents (F1461–F1470)

- [x] F1461 — .docx import via mammoth-style conversion
- [x] F1462 — HTML directory import (static site → notes)
- [x] F1463 — CSV → structured entities wizard — rows→notes with a Properties table + tag-column harvesting shipped; the interactive column-mapping wizard is the web UI layer
- [x] F1464 — OPML import (outlines, feed lists)
- [x] F1465 — ICS calendar import → timeline events
- [x] F1466 — Email (.eml/.mbox) import to notes
- [x] F1467 — Plain-text heuristics (headings, lists detection)
- [x] F1468 — Document import fixtures
- [x] F1469 — Format detection on drop (route to right importer)
- [x] F1470 — Document import docs

### Export Adapters (F1471–F1480)

- [x] F1471 — Export framework mirroring importer architecture
- [x] F1472 — Obsidian-flavored vault export
- [x] F1473 — Notion-importable export (md + csv structure)
- [x] F1474 — Logseq-compatible export
- [x] F1475 — JSON canonical export (full fidelity, documented schema)
- [x] F1476 — Static site export (read-only HTML vault)
- [x] F1477 — PDF book export (notebook → chaptered document) — print-ready HTML book (title page, TOC, page-break chapters) you print to PDF from the browser; a binary PDF renderer is deferred (no server-side PDF lib)
- [x] F1478 — Selective export by FQL query
- [x] F1479 — Round-trip fidelity tests (export→import→compare)
- [x] F1480 — Export docs

### Import UX (F1481–F1490)

- [~] F1481 — Unified import wizard: pick source, upload, map, preview, run — web UI; the server pipeline (detect → dry-run → run) it drives is shipped
- [~] F1482 — Mapping preview UI (sample of converted notes) — web UI; the dry-run report data behind it is shipped
- [~] F1483 — Progress with per-file status and pause/resume — web UI; resume is shipped (re-run a batchId), per-file progress + pause is the UI layer
- [~] F1484 — Error triage view (failed items, reasons, retry) — web UI; per-item errors are returned in the import result
- [~] F1485 — Post-import tour (where everything went) — web UI; provenance + health report provide the data
- [x] F1486 — Import health report (links resolved %, lossy conversions)
- [x] F1487 — Scheduled re-import for living sources (folder watch) — server re-sync mechanism (re-run a batch to pick up new docs) shipped + tested; the folder-watch trigger + changed-in-place updates land later
- [x] F1488 — CLI import parity for all sources
- [~] F1489 — Import UX e2e tests — needs a browser e2e runner; deferred with the web UI
- [x] F1490 — Migration-day playbooks (per-source checklists)

### Interop Epic Close (F1491–F1500)

- [x] F1491 — Cross-importer link integrity audit tool
- [x] F1492 — Performance: 50k-note import benchmark — benchmark harness shipped + tested (CI asserts a 2k import well within budget; the same harness scales to 50k via a script)
- [x] F1493 — Memory ceiling enforcement on huge imports
- [x] F1494 — Fidelity scoreboard doc (what survives per source)
- [x] F1495 — Import telemetry (local) for failure-pattern tuning
- [x] F1496 — Fixture corpus consolidation + licensing check — all test corpora are synthetic, built in-process (hand-written ZIP/ENEX/tar bytes + JSON literals); no third-party copyrighted sample data ships in the repo
- [x] F1497 — Importer fuzz pass (malformed exports never crash)
- [x] F1498 — Full interop regression suite in CI — the import/export suites run in `pnpm test` (the CI gate); 2,781 tests green
- [x] F1499 — Interop documentation hub
- [x] F1500 — Epic 15 retro devlog
...
- [x] F1601 — Local TTS engine adapter (Piper-class voices) — `audio/tts/adapter.ts` + `piper.ts`, graceful when no binary
- [x] F1602 — Voice catalog: install, preview, manage local voices — `listVoices` over the configured model dir; preview via synth
- [x] F1603 — Server-side synthesis pipeline with caching by text hash — `synthesis.ts` content-addressed cache (migration 030)
- [~] F1604 — Web Speech API fallback path — server reports `available:false` to trigger it; the browser fallback itself is a web-layer pass
- [x] F1605 — Pronunciation lexicon (names, invented words) — `lexicon.ts`
- [x] F1606 — SSML-ish markup subset (pauses, emphasis, rate) — `markup.ts`
- [x] F1607 — Synthesis queue with priority (interactive vs batch) — `SynthesisQueue`
- [x] F1608 — Voice settings per vault (default narrator) — `settings.ts` (migration 030)
- [x] F1609 — Synthesis benchmark + quality matrix doc — `docs/audio/tts.md`
- [x] F1610 — TTS adapter tests — runtime/synthesis/settings/route suites

### Voice Casting (F1611–F1620)

- [x] F1611 — Per-entity voice assignment (character → voice) — `entity_voices` (migration 031) + repo
- [x] F1612 — Dialogue attribution detection in story text — `casting/attribution.ts` (said-before/after, curly quotes, "the goblin", knownSpeakers)
- [x] F1613 — Narrator vs character line separation — `casting/separate.ts`
- [~] F1614 — Voice audition UI (hear candidates per character) — web-layer UI; server provides voices (/tts/voices) + per-entity assignment to drive it
- [x] F1615 — Per-character rate/pitch adjustments — on the voice assignment record
- [x] F1616 — Cast sheet per story (saved casting) — `cast_sheets` + repo, route PUT/GET /stories/:id/cast
- [x] F1617 — Casting templates (reuse across stories) — story-less cast sheets, /casting/templates
- [x] F1618 — Uncast-line fallback rules — `casting/resolve.ts` (narration→narrator, speaker→bySpeaker, unknown→default→narrator)
- [x] F1619 — Casting data in story manifest — `castSheets.manifest(storyId)`
- [x] F1620 — Casting tests — attribution/separate/resolve + repo + route suites (55 tests)

### Narration Renderer (F1621–F1630)

- [x] F1621 — Story path → audio scene rendering (line-by-line synthesis) — `narration/scene.ts` (Forge path → voiced SceneItems)
- [x] F1622 — Choice-point audio handling (menu voice, earcons) — earcon + choice SceneItems at branch points
- [~] F1623 — Live narration in player (speak as you read) — web player; server provides the scene + timeline it speaks
- [x] F1624 — Pre-render mode: full path baked to audio file — `narration/prerender.ts` + `wav.ts` (parse/silence/concat), route POST /stories/:id/narration/prerender
- [~] F1625 — Pause/resume/skip controls tied to story position — web player; driven by the timeline model (F1626)
- [x] F1626 — Audio position ↔ text position sync model — `narration/timeline.ts` (itemAtTime/timeOfItem)
- [~] F1627 — Speed control with pitch preservation — Web Audio (playbackRate + pitch node) in the player; rate flows through SynthesisRequest
- [~] F1628 — Sleep timer — web player UI
- [x] F1629 — Renderer performance (faster-than-realtime synthesis) — `realtimeRatio()` on the pre-render result, surfaced by the route
- [x] F1630 — Renderer tests — scene/timeline/prerender/wav + route suites

### Soundscapes (F1631–F1640)

- [~] F1631 — Layered ambient engine (loops + one-shots, Web Audio) — Web Audio engine is web; server provides the library + bindings it plays
- [x] F1632 — Scene-tag soundscape bindings (# scene: storm) — `soundscape/bindings.ts`, route POST /stories/:id/soundscape
- [x] F1633 — Crossfade and ducking under narration — ducking model `duckedAmbient()` (Web Audio crossfade is the player's)
- [x] F1634 — Bundled CC0 sound library with attribution manifest — `soundscape/library.ts`, routes /soundscape/library + /attribution
- [~] F1635 — User sound import to library — needs file upload + Web Audio decode (web); the CC0 library is the shipped layer
- [~] F1636 — Soundscape editor (compose layers per scene) — web editor UI
- [x] F1637 — Story-effect sound triggers (~ play("door")) — `soundscape/triggers.ts`
- [x] F1638 — Volume mixing panel (narration/ambient/effects) — `soundscape/mixer.ts` model + per-vault settings (migration 032), /soundscape/mix
- [~] F1639 — Audio memory management (sample unloading) — Web Audio buffer lifecycle (web)
- [x] F1640 — Soundscape tests — bindings/triggers/library/mixer/settings + route suites

### Read-Along (F1641–F1650)

- [~] F1641 — Word/sentence-level highlight sync during narration — web rendering; driven by the alignment model below
- [x] F1642 — Synthesis timestamp extraction for alignment — `readalong/align.ts` `alignmentFromBoundaries`
- [~] F1643 — Auto-scroll following narration — web; uses sentence timings
- [~] F1644 — Tap-word-to-jump audio seeking — web; uses per-word `charStart/charEnd` + `wordAtTime`/`timeOfWord`
- [~] F1645 — Karaoke mode styling options — web styling
- [x] F1646 — Read-along for plain notes (not just stories) — alignment is text-generic; route POST /readalong/align
- [x] F1647 — Alignment fallback when timestamps unavailable — `estimateAlignment` (length-weighted proportional)
- [~] F1648 — Reading-practice mode (record user, simple comparison) — web mic capture
- [~] F1649 — Read-along accessibility review — web/a11y review
- [x] F1650 — Read-along tests — align + route suites

### Recording Studio (F1651–F1660)

- [x] F1651 — Human narration recording per knot/paragraph — line-keyed takes repo + POST /stories/:id/takes
- [x] F1652 — Punch-in re-recording (fix one sentence) — per-line takes: upload a new take for one lineKey (mic punch-in UX is web)
- [x] F1653 — Take management (multiple takes, pick best) — list + setActive + promote-on-delete
- [~] F1654 — Waveform editor (trim, silence trim) — web Web Audio editor
- [~] F1655 — Noise gate / normalize processing — web DSP
- [x] F1656 — Mixed casts: human + TTS in one story — `studio/plan.ts` buildRecordingPlan
- [x] F1657 — Recording session checklist UI (lines remaining) — `sessionChecklist`; route returns it (UI is web)
- [~] F1658 — Mobile recording support (PWA mic) — web PWA mic capture
- [x] F1659 — Storage strategy for takes (opus, content-addressed) — sha256-addressed dedup repo (migration 033)
- [x] F1660 — Studio tests — plan + repo + route suites

### Audio Export (F1661–F1670)

- [x] F1661 — Path-to-audiobook export (chosen path → chapters + manifest + cue) — `audio/export/audiobook.ts` (m4b container muxing is the codec/web layer)
- [x] F1662 — Chapter markers from knot titles — `buildChapters` (titleized knot ids, contiguous-knot grouping)
- [x] F1663 — Embedded cover art and metadata — `AudiobookMetadata` (title/author/narrator/cover) carried into the manifest + cue
- [~] F1664 — MP3/opus alternative formats — format selection + per-format size estimate ship; actual encode needs a codec (web/native)
- [~] F1665 — Multi-path export (one file per major branch) — needs branch enumeration + N bakes; the per-path manifest is reusable
- [x] F1666 — Note-to-audio export (listen to any notebook) — POST /notebooks/:id/audiobook (one chapter per note)
- [~] F1667 — Export queue with progress + cancel — reuses the ingest job/queue infra; web progress UI
- [x] F1668 — Output size estimation upfront — `estimateAudioBytes` (per-format bitrate)
- [x] F1669 — Export integrity tests (duration, chapters) — audiobook manifest + route suites
- [~] F1670 — Export docs — deferred to the Epic 17 docs pass (F1696 audio user guide)

### Playback System (F1671–F1680)

- [~] F1671 — Media Session API: lock-screen controls, artwork — web/PWA (navigator.mediaSession)
- [~] F1672 — Background playback in PWA — web/PWA audio session
- [x] F1673 — Playback position persistence per story/note — `playbackRepo.position` (migration 034), GET/PUT /playback/:type/:id
- [x] F1674 — Listening queue (chain stories/notes) — `playbackRepo.queue` + /playback/queue routes
- [x] F1675 — Offline audio caching with pin controls — server pin registry + /playback/pins (byte caching is the web SW layer)
- [~] F1676 — Bluetooth/headphone control handling — web media-key events
- [~] F1677 — Interruption recovery (calls, route changes) — web audio-session events
- [x] F1678 — Listening stats (time, completion) — `position.stats()` (accumulated listened-ms + completion), GET /playback/stats
- [~] F1679 — CarPlay-adjacent web behavior verification — web verification pass
- [x] F1680 — Playback tests — repo + route suites

### Audio Accessibility (F1681–F1690)

- [~] F1681 — Full app audio-first navigation review — web review pass
- [x] F1682 — Choice reading with numbered selection by voice UI patterns — `numberedChoiceMenu` + /stories/:id/transcript
- [~] F1683 — Audio descriptions for story images — needs an image alt/description model (separate feature)
- [x] F1684 — Caption/transcript view for all audio — `a11y/transcript.ts` (`buildTranscript` + WebVTT `buildVtt`)
- [~] F1685 — Dyslexia-friendly read-along presets — web styling presets
- [x] F1686 — Volume normalization across voices — `normalizeVoices` audio setting (gain application is web DSP)
- [x] F1687 — Mono/balance options — `mono` + `balance` audio settings, PUT /soundscape/mix
- [~] F1688 — Flashing/motion-free audio visualizations — web canvas/visualization
- [~] F1689 — A11y audio review with checklist — review pass
- [x] F1690 — Accessibility tests — transcript + settings + route suites

### Audio Epic Close (F1691–F1700)

- [x] F1691 — End-to-end: cast → soundscape → narrate → export demo fable — `audio/epic-close.test.ts` pipeline
- [x] F1692 — Performance: synthesis cache hit-rate tuning — `SynthesisCache.stats()` (hitRate), GET /tts/cache
- [x] F1693 — Disk budget controls for audio caches — `cache.clear()` + `evictToFit` + cacheBudgetMb, DELETE /tts/cache
- [~] F1694 — Audio settings consolidation page — web settings UI (all settings exist server-side)
- [~] F1695 — Demo audio fable in seed content — the demo fable lives in the pipeline test; seeding it is a content pass
- [x] F1696 — Audio user guide — `docs/audio/guide.md`
- [x] F1697 — Audio regression suite — end-to-end pipeline test + the full audio suite
- [~] F1698 — Battery profiling on mobile playback — web/mobile profiling
- [~] F1699 — Audio plugin API surface — cross-cutting plugin work, deferred to its own pass
- [x] F1700 — Epic 17 retro devlog — `docs/devlog/epic-17.md`
...
- [x] F1701 — Card model: prompt/answer bound to source note blocks — `cards` table + repo (migration 035)
- [x] F1702 — FSRS scheduler implementation with parameter defaults — `learning/fsrs.ts` (FSRS-5, 19 default weights)
- [x] F1703 — Review log storage (every rating, full history) — `review_log` table + `reviewLog()`
- [~] F1704 — Scheduler parameter optimization from review history — gradient-fit optimizer is its own pass; the full log it needs is captured
- [x] F1705 — Due-queue computation with timezone correctness — `dueQueue({now})` compares due ≤ client clock
- [x] F1706 — New-card introduction limits and ordering — `newLimit` cap + created-order intake
- [x] F1707 — Suspend/bury mechanics — `setState` suspend/bury removes from queue + counts
- [x] F1708 — Scheduler property tests (intervals monotone sane) — `fsrs.test.ts` (ordering, monotonicity, clamps)
- [x] F1709 — FSRS conformance vectors test — exact forgetting-curve identities R(S,S)=0.9, I(S,0.9)=S + initial seeds
- [x] F1710 — Scheduler benchmark (100k cards) — schedules 100k under budget
...
- [x] F1761 — Sibling spacing (related cards not same session) — `edge.ts` `spaceSiblings`, GET /review/session
- [x] F1762 — Duplicate card detection — `findDuplicates` (normalised prompt), GET /cards/duplicates
- [x] F1763 — Timezone travel handling — due queue + session take the client `now` ISO, so day boundaries follow the device clock
- [x] F1764 — Vacation mode (pause without pile-up shock) — `vacationUntil` setting (migration 037); session returns empty while paused
- [x] F1765 — Catch-up strategy after long gaps — `applyCatchUp` + daily new/review caps
- [x] F1766 — Card priority overrides — `priorityOverrides` map; session stable-sorts by priority
- [x] F1767 — Maximum interval caps — `maxIntervalDays` (FSRS schedule option + deck + learning settings)
- [~] F1768 — Re-learning steps configuration — `relearningSteps` persisted; multi-step relearning execution deferred
- [x] F1769 — Edge case test matrix — edge module + settings + route suites
- [~] F1770 — Scheduler edge docs — learning docs pass (F1795)
