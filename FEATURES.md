...
**Status:** Epic 14 (AI Co-Writer & Modality Mesh) in progress — **F1301–F1370 shipped**, next up F1371 (AI command surface). Built on the pluggable AIRuntime + task router (graceful zero-AI mode F1309, versioned prompt templates, re-ask-on-schema-failure): RAG "ask your vault" (grounded + cited answers, confidence, honest no-source refusal, conversation memory, Q&A-history notes, follow-ups — F1321–F1330); note intelligence + transforms (summarize/tags/title, rewrite, outline, meeting structurer, weekly review, anti-hallucination link suggestions — F1331–F1340); story co-writer (beats, choices, scene draft, style capture, consistency check, gap analysis, provenance markers — F1341–F1350); character & dialogue AI (grounded dialogue, voice cards, polish, NPC interview, fact extraction, relationships, name gen, arc tracker — F1351–F1360); and the opt-in **Claude cloud adapter** (same adapter contract, key mgmt + masking/validation, per-feature routing, egress-consent gate, per-notebook exclusions, streaming + retry/backoff, local monthly token meter via migration 024, cache-friendly shaping — F1361–F1370, mocked-API tests with zero real network calls). ~2,464 tests green across 207 files. Deferred-with-reason this epic: F1323/F1347 UI panels, F1329/F1369 eval sets (await F1381 harness), F1364 consent dialog/indicator (web layer) — gate + endpoints all shipped. Earlier epics: Epic 12 (CRDT/collab) and Epic 13 (encrypted vault) complete; the vault keystone (thread the field codec through the notes service incl. revisions) remains queued for its own focused session (see docs/devlog/epic-14.md).
